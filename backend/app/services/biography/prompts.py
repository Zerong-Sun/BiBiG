"""Biography LLM prompts aligned with the biography-writer skill."""

from typing import List, Optional

# Shared evidence and ethics rules (from biography-writer Operating Rules)
BIOGRAPHY_WRITER_CORE_RULES = """## 传记写作原则（biography-writer）
...
def build_biography_system_prompt(style: str) -> str:
    ...
    return (
        f"你是一位专业的传记作家，遵循 biography-writer 非虚构传记规范。\n\n"
        f"{BIOGRAPHY_WRITER_CORE_RULES}\n\n"
        f"## 写作风格\n\n{style_prompt}\n\n"
...
def build_interview_questions_system_prompt(missing_info: Optional[List[str]] = None) -> str:
    ...
    return f"""你是一位温暖、善于倾听的传记采访者，遵循 biography-writer 采访规范。
...
def build_synopsis_questions_system_prompt(recording_method_hint: str = "") -> str:
    ...
    return f"""你是一位温暖的传记采访者，遵循 biography-writer 采访规范。
...
def build_missing_info_analysis_system_prompt() -> str:
    return """你是一位传记编辑，遵循 biography-writer 研究规范。
...
def build_chapter_structure_system_prompt() -> str:
    return f"""你是一位专业的传记书籍编辑，遵循 biography-writer 结构规范。
...
def build_chapter_enhance_system_prompt(chapter_title: str, chapter_summary: str = "") -> str:
    ...
    return f"""你是一位专业的传记作家，遵循 biography-writer 修订规范。
